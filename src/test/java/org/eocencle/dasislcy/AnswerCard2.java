 ...
 * @Description:
 */
public class AnswerCard2 {

//    @RequestMapping(value = "cardMarking")
    public void cardMarking(String imagefile,Integer picno) {
        try {
...
//		 Highgui.imwrite("D:\\test\\abc\\card\\card_cut5.png", cut5);
        ...
        System.out.println("学生学号为："+studentNo);
...
//		 System.out.println("-----w------"+(temp.width()*80/2693-20));
//		 System.out.println("-----h------"+(temp.height()*80/2764-20));
...
//						+ rect.height), new Scalar(0, 255, 0), 2);
        ...
        for (int i = 0; i < totoSize; i+=answerCols) {
            ...
            for (int j = 0; j < newList.size(); j++) {
                ...
                if(p100>80){
                    resultChoose += anno;
                    System.out.println(p100+" 第"+queno+"行:选项（"+anno+"）填涂");
                }else{
                    System.out.println(p100+" 第"+queno+"行:选项（"+anno+"）未填涂");
                }
